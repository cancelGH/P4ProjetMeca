...
data.b2 = 0.450; %[m]
F = -120; %[N] 
alpha = 10; beta = 0.1472; gamma = 0.9; %parametre pour transalater la courbe S
S1 = F*(tanh(alpha(data.qd(1)-beta))+gamma); %vaut entre -228 et 12
T4 = 17; %[N*m]
...
data.q = [0.332; 2*pi/3; 0.265; 0; 0]; % generalized coordinates
data.qd = [0; 0; 0; 0; 0]; % rate of generalized coordinates
data.qdd = [0; 0; 0; 0; 0];% acceleration of generalized coordinates
...
%ODE45
[temps, u] = ode45(@equaDif, [0 20], [data.q(data.qu);data.qd(data.qu)]);
